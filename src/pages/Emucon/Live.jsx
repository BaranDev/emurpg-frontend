...
// Corner type configuration with medieval green forest theme
const cornerConfig = {
  entertainment: {
...
// Status badge component with flashy animations
...
// Event row component - medieval departure board style
...
const EmuconLive = () => {
  ...
  const handleShowParticipants = async (event) => {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/api/emucon/admin/event/${event.id}/participants`
      );
      if (response.ok) {
  ...
  // Fetch data from backend (public endpoint)
  const fetchData = useCallback(async () => {
    try {
      const response = await fetch(`${backendUrl}/api/emucon/corners`);
      if (response.ok) {
        const data = await response.json();

        // Convert corners object to array
        const cornersObj = data.corners || {};
        const cornersArray = Object.values(cornersObj);
        setCorners(cornersArray);

        // Extract all events with corner and club info
        const events = [];
        cornersArray.forEach((corner) => {
          // Convert clubs object to array
          const clubsObj = corner.clubs || {};
          const clubsArray = Object.values(clubsObj);
        ...
        events.sort((a, b) => {
          ...
          // Sort by time within same status
  ...
  useEffect(() => {
    const syncInterval = setInterval(fetchData, 30 * 1000);
  ...
  // Auto-scroll effect for scrollable containers
  useEffect(() => {
    ...
    const autoScroll = () => {
      ...
      if (newScrollTop >= maxScroll && scrollDirection === 1) {
        ...
        // Reached top, move to next container
        scrollDirection = 1;
    ...
    autoScrollIntervalRef.current = setInterval(autoScroll, 50); // Smooth 50ms interval
  ...
  // Handle user scroll interaction - pause auto-scroll temporarily
  useEffect(() => {
  ...
  // Continuous events - only show live ones in separate section
  const liveContinuousEvents = allEvents.filter(
    (e) => e.status === "live" && e.eventType === "continuous"
  );

  // Regular events (scheduled + standTime, excluding continuous)
  ...
  return (
    ...
    >
      ...
      <header className="sticky top-0 z-40 bg-gradient-to-r from-forest-dark via-forest-deep to-forest-dark border-b-2 border-gold-light/30 shadow-lg shadow-black/50 backdrop-blur-sm">
        <div className="max-w-[1800px] mx-auto px-3 sm:px-4 py-2 sm:py-3 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs sm:text-sm text-emucon-text-secondary hidden sm:block">
                Real-time Event Board / Canli Etkinlik Panosu
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
            {/* Compact Stats - Live/Upcoming/Completed */}
      ...
      <main className="max-w-[1800px] mx-auto px-3 sm:px-4 pt-6 sm:pt-8 pb-16 sm:pb-20 space-y-4 sm:space-y-6">
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <h2 className="font-cinzel text-xs sm:text-lg font-bold text-white">
                  <span className="hidden sm:inline">
                    CONTINUOUS ACTIVITIES / SUREKLI ETKINLIKLER
                  </span>
        ...
        {/* LIVE Events Section - Most Prominent */}
        {liveEvents.length > 0 && (
            ...
            <div className="p-4">
              ...
              {participantsLoading ? (
                <div className="flex items-center justify-center py-8">
                  <RefreshCw className="w-6 h-6 animate-spin text-forest-light" />
                </div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-forest-dark via-forest-deep to-forest-dark border-t border-forest-light/20 py-2 px-3 sm:px-4 backdrop-blur-sm">
        <div className="max-w-[1800px] mx-auto flex items-center justify-between text-[10px] sm:text-xs text-emucon-text-secondary">
          <span className="truncate">EMUCON 2025 - Live Board</span>
      ...
      {/* Custom styles for medieval forest theme and animations */}
      <style>{`
        /* Custom scrollbar - medieval green */
