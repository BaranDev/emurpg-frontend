 ...
 */
const LevelUpChoices = ({ choiceData, onApply, onCancel, characterName }) => {
  const [selections, setSelections] = useState({});
  const [expandedChoice, setExpandedChoice] = useState(0);
  const [isApplying, setIsApplying] = useState(false);

  const { current_level, target_level, choices, features_gained, message } =
    choiceData;
  ...
  const allChoicesMade = choices?.every(
    (choice) => selections[choice.type] !== undefined,
  );
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, rgba(61, 40, 23, 0.98), rgba(42, 26, 15, 0.98))",
          border: "2px solid rgba(139, 69, 19, 0.5)",
          boxShadow:
            "0 25px 50px rgba(0, 0, 0, 0.5), 0 0 100px rgba(139, 69, 19, 0.15)",
        }}
      >
        {/* Header */}
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, rgba(139, 69, 19, 0.4), rgba(101, 50, 14, 0.3))",
          }}
        >
          {/* Decorative sparkles */}
          ...
          <div className="flex items-center gap-3 mb-2">
            <ChevronUp className="w-8 h-8 text-tavern-gold" />
            <div>
              <h2 className="text-2xl font-cinzel text-white">Level Up!</h2>
              <p className="text-sm text-tavern-parchment">
                {characterName} • Level {current_level} → {target_level}
              </p>
            </div>
          </div>

          <p className="text-tavern-parchment text-sm mt-3">{message}</p>
        </div>
        ...
        <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
              ...
              <div className="space-y-2">
                {features_gained.map((feature, i) => (
                  <div key={i} className="text-sm">
                    ...
                    <p className="text-tavern-parchmentDark text-xs mt-0.5 line-clamp-2">
                      {feature.description}
                    </p>
                  </div>
                ))}
          ...
          {(!choices || choices.length === 0) && (
            ...
            >
              <p className="text-tavern-parchment text-sm">
                No choices required for this level. Review the features above
                and click Apply to level up.
              </p>
            </div>
          )}
          ...
          {choices?.map((choice, choiceIndex) => (
            <div key={choiceIndex} className="space-y-3">
              <button
                onClick={() =>
                  setExpandedChoice(
                    expandedChoice === choiceIndex ? -1 : choiceIndex,
                  )
                }
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <span className="text-xs text-tavern-parchmentDark">
                      ...
                      className={`
                        ...
                        ${
                          selections[choice.type] === option.name
                            ? "ring-2 ring-tavern-gold bg-tavern-gold/20"
                            : "hover:bg-white/5"
                        }
                      `}
                      style={{
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          {option.description && (
                            <p className="text-xs text-tavern-parchmentDark mt-1 line-clamp-2">
                              {option.description}
                            </p>
                          )}
                        </div>
                        {selections[choice.type] === option.name && (
                          <Check className="w-4 h-4 text-tavern-gold flex-shrink-0 mt-0.5" />
                        )}
                      </div>
                    </button>
                  ))}
        ...
        >
          <button
            onClick={onCancel}
            className="px-5 py-2.5 rounded-lg text-tavern-parchment hover:bg-white/10 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            style={{
              background: allChoicesMade
                ? "linear-gradient(135deg, #ffaa33, #8b4513)"
                : "rgba(139, 69, 19, 0.3)",
              color: "white",
            }}
          >
...
LevelUpChoices.propTypes = {
  choiceData: PropTypes.shape({
    ...
    features_gained: PropTypes.arrayOf(
      PropTypes.shape({
        name: PropTypes.string,
        description: PropTypes.string,
      }),
