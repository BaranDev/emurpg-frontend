...
import { 
  User, Scroll, History, ChevronDown, ChevronUp, Download, Save,
  Heart, Brain, Zap, Shield, Skull, Star, Plus, Minus, RefreshCw,
  Edit, Sparkles, X, Send, Trash2, ArrowLeft
} from "lucide-react";
import DiceRoller from "./DiceRoller";
import { saveCharacter, getCharacters, updateCharacter } from "../../utils/characterStorage";
...
const CharrollerResults = ({ 
  ...
  // Theme colors
  const themeColors = theme === "tavern" ? {
    cardBg: "linear-gradient(135deg, rgba(61, 40, 23, 0.95), rgba(42, 26, 15, 0.95))",
    cardBorder: "rgba(139, 69, 19, 0.5)",
    accent: "#ffaa33",
    accentLight: "rgba(255, 170, 51, 0.3)",
    text: "#d4a574",
    textDark: "#a67c52",
    buttonBg: "linear-gradient(135deg, rgba(139, 69, 19, 0.6), rgba(101, 50, 14, 0.6))",
  } : {
    cardBg: "linear-gradient(135deg, rgba(30, 58, 95, 0.6), rgba(26, 45, 74, 0.8))",
    cardBorder: "rgba(74, 158, 255, 0.3)",
    accent: "#4a9eff",
    accentLight: "rgba(74, 158, 255, 0.3)",
    text: "#94a3b8",
    textDark: "#64748b",
    buttonBg: "linear-gradient(135deg, rgba(74, 158, 255, 0.3), rgba(45, 90, 135, 0.3))",
  };
  ...
  const exportCharacter = () => {
    ...
    a.href = url;
    const charName = (characterData.character_name || "character").replace(/[^a-z0-9]/gi, "_");
  ...
  const categoryOrder = ["ability", "save", "skill", "attack", "damage", "spell", 
                         "approach", "characteristic", "combat", "sanity", "other"];
  ...
  const conditionOptions = {
    ...
    pathfinder2e: ["Blinded", "Clumsy", "Confused", "Controlled", "Dazzled", "Deafened",
    ...
    <div className="w-full max-w-4xl mx-auto">
        ...
        <div className="flex items-start gap-4">
          ...
          {characterData.portrait_url ? (
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                />
                <span className="text-[10px] text-blue-300 mt-1">Generating...</span>
              </div>
              {/* CSS animations */}
              <style>{`
                @keyframes shimmer {
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-3 text-sm text-silver-light">
              {characterData.class && (
                <span>Class: <strong>{characterData.class}</strong></span>
              )}
              {characterData.level && (
                <span>Level: <strong>{characterData.level}</strong></span>
              )}
              {characterData.race && (
                <span>Race: <strong>{characterData.race}</strong></span>
              )}
              {characterData.occupation && (
                <span>Occupation: <strong>{characterData.occupation}</strong></span>
              )}
              {characterData.high_concept && (
                <span>Concept: <strong>{characterData.high_concept}</strong></span>
              )}
            </div>
      ...
      {showEditModal && (
        ...
        >
          <div 
            ...
            style={{
              background: "linear-gradient(135deg, rgba(30, 58, 95, 0.95), rgba(26, 45, 74, 0.98))",
          ...
          >
            ...
            <p className="text-sm text-silver-light mb-4">
              Describe the changes you want to make to your character. The AI will update stats, 
              abilities, and other attributes while <strong>preserving your portrait</strong>.
            </p>
      ...
      <div className="mb-6">
        ...
        >
          ...
          {showTrackers ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
        </button>
          ...
          >
            ...
            {(system === "dnd5e" || system === "pathfinder2e") && (
              <>
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <div className="flex items-center gap-1">
                    <button 
                      onClick={() => adjustValue("currentHP", -1, 0, trackers.maxHP + trackers.tempHP)}
                    ...
                    <input
                      ...
                      onChange={(e) => updateTracker("currentHP", parseInt(e.target.value) || 0)}
                    ...
                    <input
                      ...
                      onChange={(e) => updateTracker("maxHP", parseInt(e.target.value) || 0)}
                      className="w-16 text-center bg-slate-800 border border-slate-600 rounded px-2 py-1 text-white"
                    />
                    <button 
                      onClick={() => adjustValue("currentHP", 1, 0, trackers.maxHP + trackers.tempHP)}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => updateTracker("tempHP", parseInt(e.target.value) || 0)}
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {[0, 1, 2].map(i => (
                        <button
                          key={`succ-${i}`}
                          onClick={() => updateTracker("deathSaveSuccess", i < trackers.deathSaveSuccess ? i : i + 1)}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {[0, 1, 2].map(i => (
                        <button
                          key={`fail-${i}`}
                          onClick={() => updateTracker("deathSaveFail", i < trackers.deathSaveFail ? i : i + 1)}
                          className={`w-6 h-6 rounded-full border-2 ${
                  ...
                  <div className="flex flex-wrap items-center gap-6">
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        onChange={(e) => updateTracker("focusPoints", parseInt(e.target.value) || 0)}
                        className="w-12 text-center bg-slate-800 border border-slate-600 rounded px-2 py-1 text-white"
                      />
                      <span className="text-silver-dark">/</span>
                      <span className="text-silver-light">{trackers.maxFocusPoints}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {[0, 1, 2].map(i => (
                        <button
                          key={`hero-${i}`}
                          onClick={() => updateTracker("heroPoints", i < trackers.heroPoints ? i : i + 1)}
                ...
                <div className="flex flex-wrap items-center gap-6">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => updateTracker("currentSanity", parseInt(e.target.value) || 0)}
                      className="w-16 text-center bg-slate-800 border border-slate-600 rounded px-2 py-1 text-white"
                    />
                    <span className="text-silver-dark">/</span>
                    <span className="text-silver-light">{trackers.maxSanity}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => updateTracker("currentLuck", parseInt(e.target.value) || 0)}
                      className="w-16 text-center bg-slate-800 border border-slate-600 rounded px-2 py-1 text-white"
                    />
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => updateTracker("currentMP", parseInt(e.target.value) || 0)}
                      className="w-16 text-center bg-slate-800 border border-slate-600 rounded px-2 py-1 text-white"
                    />
                    <span className="text-silver-dark">/</span>
              ...
              <>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="w-12 text-center text-xl font-bold text-white">{trackers.fatePoints}</span>
                    <button 
                  ...
                  <span className="text-xs text-silver-dark">(Refresh: {trackers.maxRefresh})</span>
                </div>

                {/* Stress Boxes */}
                <div className="flex flex-wrap gap-6">
                  <div>
                    <span className="text-sm text-silver-light block mb-2">Physical Stress</span>
                  ...
                  <div>
                    <span className="text-sm text-silver-light block mb-2">Mental Stress</span>
                    <div className="flex gap-2">
                ...
                <div className="space-y-2">
                  <span className="text-sm text-silver-light block">Consequences</span>
                  {["mild", "moderate", "severe"].map(level => (
                    <div key={level} className="flex items-center gap-2">
                      <span className="text-xs text-silver-dark w-20 capitalize">{level} (-{level === "mild" ? 2 : level === "moderate" ? 4 : 6}):</span>
                      <input
                        type="text"
                        value={trackers.consequences[level]}
                        onChange={(e) => setTrackers(prev => ({
                          ...prev,
                          consequences: { ...prev.consequences, [level]: e.target.value }
                        }))}
                        placeholder="Empty"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <select
                  onChange={(e) => {
                    if (e.target.value && !trackers.conditions.includes(e.target.value)) {
                      setTrackers(prev => ({
                        ...prev,
                ...
                >
                  ...
                  {(conditionOptions[system] || conditionOptions.dnd5e).map(cond => (
            ...
            {/* Notes */}
            <div>
              <span className="text-sm text-silver-light block mb-2">Session Notes</span>
              <textarea
          ...
          {rollHistory.map((roll, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <span 
                  className={`font-bold ${
                    roll.isCriticalSuccess ? "text-yellow-400" :
                    roll.isCriticalFail ? "text-red-500" :
                    "text-white"
                  }`}
                >
                  {roll.total}
                </span>
                <span className="text-xs text-silver-dark">{roll.timestamp}</span>
              </div>
            </div>
          ))}
        </div>
            ...
            <div key={category}>
              ...
              >
                <span className="font-medium">
                  {categoryLabels[category] || category} ({groupedRolls[category].length})
                </span>
...
CharrollerResults.propTypes = {
  characterData: PropTypes.shape({
    ...
    roll_list: PropTypes.arrayOf(PropTypes.shape({
